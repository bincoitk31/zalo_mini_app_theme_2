        ...
        {order.order_items.map((el, idx) => (
          <>
            <div className={idx == order.order_items.length - 1 ? 'py-4 justify-between' : 'py-4 justify-between border-b boder-b-solid border-b-[#f2f2f2]'} key={idx}>
              <div className="flex">
                <div className="min-w-[60px] w-[60px] h-[60px]" onClick={() => navigate(`/product/${el.variation_info.product_id}`)}>
                ...
                <div className="px-2 flex flex-col justify-between w-full">
                  <div className="pb-1 font-medium" onClick={() => navigate(`/product/${el.variation_info.product_id}`)}>{el.variation_info.name}</div>
